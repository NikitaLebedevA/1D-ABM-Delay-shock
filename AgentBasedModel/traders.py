...
class SingleTrader(Trader):
    ...
    def equity(self):
        price = self.markets.price()
        return self.cash + self.assets * price

    def income(self):
        self.cash += self.assets * self.markets.dividend()      # Dividend payments
        self.cash += self.cash   * self.markets.risk_free_rate  # Interest payment

    def _buy_limit(self, quantity, price):
        order = Order(round(price, 1), round(quantity), 'bid', self)
        self.orders.append(order)
        self.markets.limit_order(order)
    ...
    def _buy_market(self, quantity) -> int:
        ...
        """
        if not self.markets.order_book['ask']:
            return quantity
        
        price = self.markets.order_book['ask'].last.price
        order = Order(price, round(quantity), 'bid', self)
        return self.markets.market_order(order).qty

    def _sell_market(self, quantity) -> int:
        """
        :return: quantity unfulfilled
        """
        if not self.markets.order_book['bid']:
            return quantity
        
        price = self.markets.order_book['bid'].last.price
        order = Order(price, round(quantity), 'ask', self)
        return self.markets.market_order(order).qty

    def _cancel_order(self, order: Order):
        self.markets.cancel_order(order)
...
class Random(SingleTrader):
    ...
    @staticmethod
    def draw_price(self, order_type, info, std: float | int = 2.5) -> float:
        ...
        # Within the spread
        if random_state < .35:
            return random.uniform(info.spreads[0][-self.lag-1]['bid'], info.spreads[0][-self.lag-1]['ask'])
            
        # Out of spread
        else:
            delta = Random.draw_delta(std)
            if order_type == 'bid':
                return info.spreads[0][-self.lag-1]['bid'] - delta
            if order_type == 'ask':
                return info.spreads[0][-self.lag-1]['ask'] + delta
    ...
    def call(self, info):
        spread = info.spreads[0][-self.lag-1]
        if spread is None:
            return
        ...
        # Limit order
            ...
            price = self.draw_price(self, order_type, info)
            quantity = self.draw_quantity()
            if order_type == 'bid':
...
class Fundamentalist(SingleTrader):
    ...
    def call(self, info):
        divs = self.markets.dividend(self.access)
        rf = self.markets.risk_free_rate

        pf = round(self.evaluate(divs, rf), 1)  # fundamental price
        p = info.prices[0][-1-self.lag]
        spread = info.spreads[0][-1-self.lag]
        t_cost = self.markets.transaction_cost
        ...
        if random_state > .45:
            ...
            ask_t = round(info.spreads[0][-self.lag-1]['ask'] * (1 + t_cost), 1)
            bid_t = round(info.spreads[0][-self.lag-1]['bid'] * (1 - t_cost), 1)
    ...
    def call(self, info):
        ...
        if self.sentiment == 'Optimistic':
            ...
            # Limit order
                ...
                self._buy_limit(
                    Random.draw_quantity(),
                    Random.draw_price(self, 'bid', info) * (1 - t_cost)
                )
            # Cancel order
            elif random_state < .35:
                if self.orders:
            ...
            # Limit order
                ...
                self._sell_limit(
                    Random.draw_quantity(),
                    Random.draw_price(self, 'ask', info) * (1 + t_cost)
                )
            # Cancel order
            elif random_state < .35:
                if self.orders:
...
class Chartist2D(MultiTrader):
    ...
    def change_sentiment(self, info, a1=1, a2=1, v1=.1):
        ...
        for idx in self.markets.keys():
            dp = (
                info.prices[idx][-self.lag-1] - info.prices[idx][-self.lag-2]  # price derivative
...
class Universalist(Fundamentalist, Chartist1D):
    ...
    def change_strategy(self, info, a1=1, a2=1, a3=1, v1=.1, v2=.1, s=.1):
        ...
        dp = (
            info.prices[self.market.id][-lag-1] - info.prices[self.market.id][-lag-2]              # price derivative
            if len(info.prices[self.market.id]) > 1 else 0
        )
        p = info.prices[self.market.id][-lag-1]                                                           # market price
