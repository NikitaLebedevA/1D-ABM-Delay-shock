...
def plot_price_fundamental(
    ...
    plt.figure(figsize=figsize)
    plt.title(
              'Рыночная и фундаментальная цена' if rolling == 1
        else f'Рыночная и фундаментальная цена (MA {rolling})'
    )
    plt.xlabel('Итерация')
    plt.ylabel('Цена')
    ...
    if idx is not None:
        ...
        plt.plot(iterations, m_values, color='tab:blue', alpha=1,  ls='-',  label=f'{exchange.name}: рыночная')
        plt.plot(iterations, f_values, color='black',    alpha=.6, ls='--', label=f'{exchange.name}: фундаментальная')
    ...
    # plot N exchanges
        ...
        for k, v in info.exchanges.items():
            ...
            iterations = range(rolling - 1, len(m_values) + rolling - 1)
            plt.plot(iterations, m_values, ls='-', label=f'рыночная цена')

        f_values = math.rolling(info.fundamental_value(0, access), rolling)    # fundamental prices
        plt.plot(iterations, f_values, color='black', alpha=.6, ls='--', label=f'фундаментальная цена')
...
def plot_price_fundamental_m(
    ...
    if idx is not None:
        ...
        plt.plot(iterations, m_values, color='tab:blue', alpha=1, ls='-',   label=f'{exchange.name}: Рыночная цена')
        plt.plot(iterations, f_values, color='tab:blue', alpha=.4, ls='--', label=f'{exchange.name}: Фундаментальная цена')
    ...
    # plot N exchanges
        ...
        for k, v in info.exchanges.items():
            ...
            c = next(colors)
            plt.plot(iterations, m_values, color=c, alpha=1, ls='-',   label=f'{v.name}: Рыночная цена')
            plt.plot(iterations, f_values, color=c, alpha=.4, ls='--', label=f'{v.name}: Фундаментальная цена')
